...
> **SECOND AMENDMENT (2026-07-06, post-execution record):** the unmask loop went one layer deeper than Step 2b. With 196 and 534 fixed, `play-canvas.tsx:111` unmasked — `react-hooks/set-state-in-effect` on the "Reset drawing state when drawingRoute mode is turned off" effect. Fixed (controller-authorized, same sanctioned idiom as Step 2b): the effect became an adjust-state-during-render block (`prevDrawingRoute` compare; on `drawingRoute` turning falsy, reset `drawingPlayerId`/`cursorPos`). Loop terminated there — file at 0 errors; ResizeObserver/measure effect never flagged, never touched. Total for this task: SEVEN errors (5 baseline-visible + 2 unmasked). Also: Step 4's `() => "dark"` server-snapshot lambda widens to `string` and fails tsc — the executed fix annotates it `(): Theme => "dark"`.
