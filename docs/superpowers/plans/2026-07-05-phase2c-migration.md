...
**2. Placeholder scan.** No "TBD/handle-appropriately" placeholders — every step lists exact before→after strings or a binding decision (D1–D15). Both judgment-heavy calls are adjudicated: D12 (play-type palette → KEEP as an allowlisted exception, team-lead decision) and D7 (mode banners → remap the `ModeButton` `activeClass` map values only, no type change).

**3. Type/name consistency.** Token utility names are used identically across all tasks (`text-muted-foreground`, `bg-secondary`, `text-primary-emphasis`, `bg-offense`, `surface-2`, …). `surface-1`/`surface-2` are always the plain-class form. `text-primary-foreground` is the single on-color foreground everywhere (D2). The `ModeButton` change is class-strings-only — remap the `activeClass` map values; keys and the `color` union stay unchanged (D7 / Task 6 Step 4) — so there is no symbol rename in the default path. Overlap resolutions guarantee each file is edited in exactly one task.
