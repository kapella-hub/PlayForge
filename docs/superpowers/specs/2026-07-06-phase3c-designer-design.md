...
- **`n()` → `detectRouteType` rename** (`src/engine/constants.ts` + its play-canvas call site): the route-type detector is exported under a meaningless single-letter name; rename to what every doc already calls it. Pure rename, zero behavior.
